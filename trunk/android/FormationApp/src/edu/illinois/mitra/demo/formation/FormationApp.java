...
//		settings.ROBOT_RADIUS(400);
	...
	public List<Object> callStarL() {
		...
		{
		//	dsm.createMW("posX" + othername, 0);
		//	dsm.createMW("posY" + othername, 0);
		}
		while(true) {
			position = gvh.gps.getMyPosition();
			
		
			// send my position to dsm.
			switch(stage) {
			...
			case PICK:
				...
				dsm.put("posX"+name, "*", position.getX());
				dsm.put("posY"+name, "*", position.getY());
				...
				for(String othername : gvh.id.getParticipants())
				{
						...
						{
							//String x1 = dsm.get("mypos", othername,"x");
							//String y1 = dsm.get("mypos", othername, "y");
				...
				if (counter == 0 )
			    {
							...
							// go to a random destination if it is the first node.
				}
				else
				{
					int moveY,moveX;
	...
	protected void receive(RobotMessage m) {
		String id = m.getFrom();
		int id_num = Integer.parseInt(id.substring(3,name.length())); // the Index
		MessageContents msg_content = m.getContents();
		
		if(m.getMID() == REQUEST_MSG){

		    ItemPosition  tmp;
			String pos = msg_content.toString();
			tmp.fromMessage(pos);
			int i;
			for (i = 0;i<5;i++)
			{
				if ( i!= id_num )
				{
					if ( (flags[i] == 1)  )
					{
						if(checkRange(tmp,destArray[i]) == 0)
						{
							RobotMessage inform = new RobotMessage(id, name, REPLY_MSG, "Fail");
			...
			if (i == 5 )
			{
				RobotMessage inform = new RobotMessage(id, name, REPLY_MSG, "OK");
				gvh.comms.addOutgoingMessage(inform);
				flags[id_num]=1;
