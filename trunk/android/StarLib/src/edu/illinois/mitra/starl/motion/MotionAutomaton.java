 ...
 * motion parameters. Includes optional collision avoidance which is controlled by the motion parameters setting.
				...
				if(!colliding && stage != null) {
					...
					case SMALLTURN:
						if(stage != prev) {
							...
							if(absangle <= A_smallturn) next = STAGE.STRAIGHT;
							if(absangle > param.ARCANGLE_MAX) next = STAGE.TURN;
							if(distance <= R_goal) next = STAGE.GOAL;
						}
						break;
