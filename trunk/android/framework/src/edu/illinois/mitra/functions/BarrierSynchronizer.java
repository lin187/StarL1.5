...
public class BarrierSynchronizer implements Synchronizer {
	...
	public boolean barrier_proceed(String barrierID) {
		...
		for(RobotMessage recd: gvh.getIncomingMessages(LogicThread.MSG_BARRIERSYNC)) {
			...
			if(barriers.containsKey(bID)) {
				...
				Log.d(TAG, "Received barrier notice for bID " + bID + " from " + recd.getFrom() + ". Current count: " + barriers.get(bID));
			} else {
